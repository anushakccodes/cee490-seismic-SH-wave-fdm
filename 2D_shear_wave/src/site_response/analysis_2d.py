...
def theoretical_2d_arrival_time(source_x, source_z, receiver_x, receiver_z, vs, peak_time_s):
...
def _pick_peak_arrival(time_s, signal, expected_t, window_s=0.4):
    """Return the time of the first local extremum exceeding 60% of the windowed peak."""
    ...
    candidates = np.where(sign_change)[0] + 1
...
def arrival_time_verification_2d(result, receiver_name):
    """Compare numerical peak arrival against t_theory = t0 + d/Vs for one receiver."""
...
def run_arrival_verification_table(result):
    checks = []
...
def format_arrival_table(checks):
...
def surface_fourier_spectrum(result, receiver_name):
    """Return (frequencies_hz, normalized amplitude spectrum) for one receiver."""
    signal = result.receiver_records_m_s[receiver_name]
...
def compute_2d_amplification(result_ref, result_layered, receiver_name, f_min=0.5, f_max=15.0):
    """Return (frequencies, A(f)) = |layered| / |ref|; NaN outside [f_min, f_max]."""
