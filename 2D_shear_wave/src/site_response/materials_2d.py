...
class Simulation2DConfig:
    ...
    @property
    def source_peak_time_s(self):
        return 1.5 / self.dominant_frequency_hz

    @property
    def figures_2d_dir(self):
        return self.project_root / "figures"

    @property
    def results_2d_dir(self):
...
class Material2D:
    ...
    interface_depth_profile_m: np.ndarray | None = None  # x-varying boundary (overrides interface_depth_m when set)
...
def _shear_modulus(density, vs):
    return density * vs**2


def homogeneous_2d_model(config):
...
def basin_2d_model(config, base_depth_m=20.0, extra_depth_m=60.0, basin_half_width_m=200.0):
    ...
    return Material2D(
        ...
        interface_depth_profile_m=basin_depth,
    )


def stress_grid_mu_x(material):
...
def format_2d_parameter_summary(config):
...
def ensure_2d_output_directories(config):
