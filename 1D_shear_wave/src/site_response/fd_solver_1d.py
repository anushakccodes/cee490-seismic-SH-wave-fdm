...
class SimulationResult:
    ...
    stress_wavefield_pa: np.ndarray
...
def nearest_grid_index(depth_m, requested_depth_m):
    """Return the grid index closest to a requested depth."""
    return int(np.argmin(np.abs(depth_m - requested_depth_m)))


def build_bottom_damping_mask(depth_m, damping_thickness_m, damping_strength):
    """Gaussian taper applied near the bottom boundary to absorb outgoing waves."""
...
def run_velocity_stress_solver(
    ...
    material,
    source_depth_m=40.0,
    receiver_depth_m=260.0,
    store_every=2,
    source_scale=2.0e-4,
):
    """Staggered-grid leapfrog solver for rho*dv/dt = d tau/dz and d tau/dt = mu*dv/dz."""
    if store_every < 1:
    ...
    stored_velocity_profiles = []
    stored_stress_profiles = []
    stored_time_values = []
    ...
    for step_index, current_time_s in enumerate(time_s):
        # Velocity update: rho*dv/dt = d tau/dz; tau=0 at free surface (z=0)
        ...
        # Stress update: d tau/dt = mu * dv/dz
        ...
        if step_index % store_every == 0:
            ...
            stored_stress_profiles.append(shear_stress.copy())
    ...
    return SimulationResult(
        ...
        stress_wavefield_pa=np.asarray(stored_stress_profiles),
