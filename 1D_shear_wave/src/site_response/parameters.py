...
class SimulationConfig:
    ...
    soil_layer_thickness_m: float = 80.0
    ...
    absorbing_layer_thickness_m: float = 50.0
    absorbing_strength: float = 0.50
    source_depth_m: float = 150.0
    receiver_depth_m: float = 0.0
    project_root: Path = Path(__file__).resolve().parents[2]

    @property
    def number_of_depth_nodes(self):
    ...
    @property
    def maximum_shear_velocity_m_s(self):
        return max(self.soil_shear_velocity_m_s, self.rock_shear_velocity_m_s)

    @property
    def minimum_shear_velocity_m_s(self):
...
def format_parameter_summary(config):
