...
def plot_convergence(dz_values: np.ndarray, errors: np.ndarray, output_path: Path | str | None = None) -> plt.Figure:
    ...
    ax.loglog(dz_values, errors, "o-", color=PROJECT_COLORS["deep_blue"], label="numerical error")
    # Reference slope-2 line anchored at coarsest point.
    ref_x = np.array([dz_values.min(), dz_values.max()])
    ref_y = errors[-1] * (ref_x / dz_values[-1]) ** 2
    ax.loglog(ref_x, ref_y, "--", color=PROJECT_COLORS["gray"], label="slope 2 reference")
...
def plot_amplification(
    ...
    apply_project_style()
    fig, ax = plt.subplots(figsize=(7.0, 3.8))
    ax.plot(frequencies_hz, amplification, color=PROJECT_COLORS["deep_blue"], label="A(f)")
    ax.axhline(1.0, color=PROJECT_COLORS["gray"], linewidth=0.8, linestyle="--")
...
def make_wave_animation(
    ...
    step = max(1, n_stored // max_frames)
    frame_indices = list(range(0, n_stored, step))
...
def make_layered_reflection_animation(
    ...
    Focuses the depth window on the soft layer and immediate surroundings to
    make reflection/reverberation visible.
    """
    apply_project_style()
    n_stored = result.velocity_wavefield_m_s.shape[0]
    step = max(1, n_stored // max_frames)
