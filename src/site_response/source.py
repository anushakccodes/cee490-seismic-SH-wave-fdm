...
def ricker_wavelet(time_s: np.ndarray, dominant_frequency_hz: float, peak_time_s: float) -> np.ndarray:
...
def build_source_time_function(config: SimulationConfig) -> tuple[np.ndarray, np.ndarray]:
    """Return the default Ricker source sampled on the simulation time axis."""
...
def normalized_amplitude_spectrum(time_s: np.ndarray, signal: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
